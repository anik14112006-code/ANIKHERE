...
import openpyxl, xlrd, psycopg2, psycopg2.extras
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS tg_users(telegram_id TEXT PRIMARY KEY,username TEXT,first_name TEXT NOT NULL,balance NUMERIC(12,2) NOT NULL DEFAULT 0,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
        CREATE TABLE IF NOT EXISTS submissions(id SERIAL PRIMARY KEY,telegram_id TEXT NOT NULL,file_id TEXT NOT NULL,file_unique_id TEXT NOT NULL,file_name TEXT NOT NULL,row_count INT NOT NULL,status TEXT NOT NULL DEFAULT 'pending',admin_note TEXT,group_message_id INT,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
        CREATE TABLE IF NOT EXISTS withdrawals(id SERIAL PRIMARY KEY,telegram_id TEXT NOT NULL,amount NUMERIC(12,2) NOT NULL,method TEXT NOT NULL DEFAULT 'bkash',account_number TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'pending',created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
    """)
...
def _q(sql,params=None,fetch="one"):
    ...
    conn.commit(); cur.close(); conn.close(); return row

def db_get_user(tid): return _q("SELECT * FROM tg_users WHERE telegram_id=%s",(str(tid),))
def db_get_or_create(tid,fname,uname=None):
    row=db_get_user(tid)
    if not row: row=_q("INSERT INTO tg_users(telegram_id,first_name,username) VALUES(%s,%s,%s) RETURNING *",(str(tid),fname,uname))
    return row
def db_set_balance(tid,bal): _q("UPDATE tg_users SET balance=%s WHERE telegram_id=%s",(bal,str(tid)),fetch="none")
def db_insert_sub(tid,fid,fuid,fname,rc): return _q("INSERT INTO submissions(telegram_id,file_id,file_unique_id,file_name,row_count) VALUES(%s,%s,%s,%s,%s) RETURNING *",(str(tid),fid,fuid,fname,rc))
def db_set_sub_group_msg(sid,gid): _q("UPDATE submissions SET group_message_id=%s WHERE id=%s",(gid,sid),fetch="none")
def db_get_sub(sid): return _q("SELECT * FROM submissions WHERE id=%s",(sid,))
def db_set_sub_status(sid,st): _q("UPDATE submissions SET status=%s,updated_at=NOW() WHERE id=%s",(st,sid),fetch="none")
def db_insert_wd(tid,amt,method,acc): return _q("INSERT INTO withdrawals(telegram_id,amount,method,account_number) VALUES(%s,%s,%s,%s) RETURNING *",(str(tid),amt,method,acc))
def db_get_wd(wid): return _q("SELECT * FROM withdrawals WHERE id=%s",(wid,))
def db_set_wd_status(wid,st): _q("UPDATE withdrawals SET status=%s WHERE id=%s",(st,wid),fetch="none")
def db_list_users(n=20): return _q("SELECT * FROM tg_users ORDER BY created_at DESC LIMIT %s",(n,),fetch="all")
def db_list_pending_subs(n=10): return _q("SELECT * FROM submissions WHERE status='pending' ORDER BY created_at DESC LIMIT %s",(n,),fetch="all")
def db_list_pending_wds(n=10): return _q("SELECT * FROM withdrawals WHERE status='pending' ORDER BY created_at DESC LIMIT %s",(n,),fetch="all")
...
async def handle_callback(update,ctx):
    ...
    if data.startswith("wd_method_"):
        ...
        await q.answer(); await q.message.reply_text(f"{emoji} *{ml}* সিলেক্ট।\n\nআপনার {al} লিখুন:",parse_mode="Markdown",reply_markup=CANCEL_KB); return
...
async def do_admin_cmd(update,ctx):
    ...
    if cmd=="/stats":
        tu=_q("SELECT COUNT(*) AS c FROM tg_users",fetch="one")["c"]
        ts=_q("SELECT COUNT(*) AS c FROM submissions",fetch="one")["c"]
        as_=_q("SELECT COUNT(*) AS c FROM submissions WHERE status='approved'",fetch="one")["c"]
        ps=_q("SELECT COUNT(*) AS c FROM submissions WHERE status='pending'",fetch="one")["c"]
        tw=_q("SELECT COALESCE(SUM(amount),0) AS s FROM withdrawals WHERE status='approved'",fetch="one")["s"]
        pw=_q("SELECT COUNT(*) AS c FROM withdrawals WHERE status='pending'",fetch="one")["c"]
        pa=_q("SELECT COALESCE(SUM(amount),0) AS s FROM withdrawals WHERE status='pending'",fetch="one")["s"]
        tb=_q("SELECT COALESCE(SUM(balance),0) AS s FROM tg_users",fetch="one")["s"]
